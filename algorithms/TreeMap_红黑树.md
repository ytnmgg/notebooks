...
删除当前节点，等价于用后继B节点的kv值覆盖到当前节点，然后再删除后继B节点。此时删除后继B节点，如果其子节点C节点存在，则等价于情况2，否则等价于情况1。所以，情况3可以先覆盖值，再等价于情况1或情况2处理。
...
private void deleteEntry(Entry<K,V> p) {
    ...
    if (replacement != null) {
        ...
        // 情况1：进入这里，表示待删节点没有子节点，直接删除p就行，但是这里和调整做了点合并逻辑，写到一起了
        
        // 先调整再删，否则删了之后就没调整的节点了
...
## 删除节点后进行调整
从上面deleteEntry代码可以看到，进入fixAfterDeletion调整逻辑，有两个入口：

- 情况1：x是替换节点（后继节点的子节点），x不一定有子节点
- 情况2：x是待删节点，且x一定没有子节点
...
所以情况1进入fixAfterDeletion调整逻辑和情况2一样，入参X节点都是没有子节点的。

- 分析1. x是红色的
    - 1-1 情况1：因为x是s的替换节点，原s是黑色的，删掉之后少了个黑色，把x替换到s位置，且颜色换成黑色即可替换原黑色s节点
    - 1-2 情况2：x是待删除节点，又是红色的，直接删即可，不会影响黑平衡
- 分析2. x是黑色的，只有可能是情况2了，因为前面分析了情况1进来的一定是红色。所以x是需要删除的节点，删了肯定影响黑平衡，需要通过左旋或右旋改变树结构实现再平衡，因为被删节点x的左右子树都为空，自己怎么旋都无用，只能考虑其父和兄弟节点的旋转了。
...
- 分析2-3：兄弟黑色的，父亲黑色的，此时兄弟的子节点不可能为黑，否则不黑平衡。假设兄弟节点没有孩子，则把兄弟节点变成红色，x节点可以删除，但是还需要继续处理黑平衡问题（x路径和兄弟路径都少了1个黑色）
- 分析2-4：兄弟黑色的，父亲颜色任意，兄弟有一个左红孩，可以将兄弟右旋，等价成下面2-5的右红孩处理。
- 分析2-5：兄弟黑色的，父亲颜色任意，兄弟有一个右红孩，此时将父节点和兄弟节点颜色互换，兄弟的右孩变黑，再将父节点左旋，x节点可以直接删除，不改变黑平衡。
- 分析2-6：兄弟黑色的，父亲颜色任意，兄弟有左右两个红孩，此时将父节点和兄弟节点颜色互换，兄弟的右孩变黑，再将父节点左旋，x节点可以直接删除，不改变黑平衡。


```java
private void fixAfterDeletion(Entry<K,V> x) {
    
    // x可能是情况2的替换节点，也可能是情况1的待删节点
    while (x != root && colorOf(x) == BLACK) {
        ...
        if (x == leftOf(parentOf(x))) {
            ...
            // 分析2-1：兄弟节点存在且为红色，旋转等价为兄弟节点是黑色的处理
            if (colorOf(sib) == RED) {
                setColor(sib, BLACK);
            ...
            if (colorOf(leftOf(sib))  == BLACK &&
                ...
                // 分析2-2和2-3，兄弟黑色，兄弟没有子节点
                setColor(sib, RED);

                // 分析2-2，父节点为红色，x指向父亲后，无法进入while循环再平衡，到最后一行被改为黑色然后结束，实际效果还是该路径新增了一个黑色（父亲）从而补偿被删的黑色，达到黑平衡
                // 分析2-3，父亲已经是黑色了，观察其它分析场景，我们需要删除某个黑色节点，实际上都是在目标节点的路径上新增了一个黑色节点以弥补缺失的黑色节点。但是2-3这个场景无法再找到黑色节点过来补充，将兄弟改红色后，x路径和兄弟路径实际上都少了一个黑色节点（虽然这两个路径看似平衡，但和其它路径比可能就少了黑色，不平衡了）。所以只能将x指向父节点，把父节点当做待删除的黑色节点进行再平衡，从其它上级路径找黑色过来补充黑色节点的损失。
                x = parentOf(x);
            } else {

                // 分析2-4，兄弟黑色，兄弟没有右孩，右旋兄弟，等价为有右孩场景处理
    ...
    // 分析1，需要染黑兼顾情况1和情况2，维持情况1的黑平衡（情况2反正要删，红黑无所谓）
    setColor(x, BLACK);
}
```
