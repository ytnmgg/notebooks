...
- 字段是唯一索引时，只有查询某个范围记录或者一条不存在的记录的时候，才会产生间隙锁，指定给某条存在的记录加锁的时候，只会加记录锁，不会产生间隙锁；
- 字段是普通索引时，不管是锁住单条，还是多条记录，都会产生间隙锁；
- 可重复读级别(RR)下才会有间隙锁！！！！
- 在RC隔离级别下，通常间隙锁会消失，降级为记录锁，所以在RC隔离级别下能够提高并发写入的性能。
...
select for update，锁不存在的记录：
    ...
    RR级别，锁期望行的前一个有记录的行、后一个有记录的行，之间的间隙

## 半同步复制
半同步复制，指主库收到任意从库，binlog复制进入relaylog后，即返回事物提交
- 早期是after_commit模式，即主库在二阶段提交的redolog打commit标记后，再去等待从库的ack，这种方式的风险是，commit之后，本机的其他事物就能读到提交结果，此时主库crash，主备切换后，之前在原主库能读到的信息，在新主库读不到了（binlog还未成功同步），发生了幻读。
- 后面都改成after_sync模式，即主库binlog写入以后，先等从库的ack消息，等到以后，再commit事务，未收到ack就不提交事务。这样就避免了幻读。
- 半同步复制过程中，如果接受ack超时，会降级成异步复制模式。

### commit异步化 优化
官方版本，事物发起线程需要等待ack之后再提交事务，时间浪费在从库处理日志及网络传输ack上面。
改造版本，新增事物提交处理线程，原事务发起线程无需等待ack，回收到线程池，继续处理其他事物请求；事物提交处理线程负责接收ack和提交事物。
改造后大大提高了整体事物吞吐能力。

另外一个优化：新增最大保护模式，异步模式等待ack超时，master进入拒绝服务状态。

## 高可用
- mysql原生的主从复制（半同步）
- ADHA 数据库高可用管理系统
  - mysql服务器上装agent，探测db服务（解决了之前用中心化console服务探测mysql，面对8k～9k个实例的mysql集群压力过大问题，现在一个实例一个agent）
  - console做元数据管理，容灾切换等操作；console本身无状态，元数据保存于zk集群上面；console自身是集群部署的。
  - agent上报的心跳信息（通过简单的select操作进行）是上报到zk集群的（临时节点）。console通过对zk内容的检查（临时节点是否消失），发现有问题的mysql服务节点，发起主备切换。
  - 切换流程：console修改配置中心保存的主库地址，配置中心推送新的主库地址（之前的备库）到app应用上面（或者推送到zdal等数据库连接中间件上面），app使用最新的主库地址连接数据库。
- x-cluster raft协议强一致
  - 在原生mysql的server层和存储引擎层之间，插入了一层分布式协调层
  - 事物提交之前，leader的协调层把分布式日志（主要是binlog）转发给集群的follower，多数follower日志落盘后，leader再提交事物；否则回滚事务
  - follower的协调层收到leader转发的binlog，将其解析成数据变更，更新follower本身的数据。
